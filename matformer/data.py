...
def load_pyg_graphs(
    ...
    def atoms_to_graph(atoms):
        ...
        # pdb.set_trace()
        structure = Atoms.from_dict(atoms)#.pymatgen_converter()
        # structure = 
...
def get_torch_dataset(
    ...
    """Get Torch Dataset."""
    df = pd.DataFrame(dataset)[0:1000]
    # print("df", df)
...
def get_train_val_loaders(
    ...
    split_seed: int = 123,  #123 default
    workers: int = 0,
    ...
    if mp_id_list is not None:
        if mp_id_list == 'bulk':
            ...
            with open('mp_bulk/bulk_megnet_train.pkl', 'rb') as f:
                dataset_train = pk.load(f)#[0:1000]
            with open('mp_bulk/bulk_megnet_val.pkl', 'rb') as f:
                dataset_val = pk.load(f)#[0:424]
            with open('mp_bulk/bulk_megnet_test.pkl', 'rb') as f:
                dataset_test = pk.load(f)#[0:424]
        
        if mp_id_list == 'shear':
            print('using mp shear dataset')
            with open('mp_shear/shear_megnet_train.pkl', 'rb') as f:
                dataset_train = pk.load(f)
            with open('mp_shear/shear_megnet_val.pkl', 'rb') as f:
                dataset_val = pk.load(f)
            with open('mp_shear/shear_megnet_test.pkl', 'rb') as f:
        ...
        ids_train_val_test["id_val"] = 0.0
        # std_train = 1.0] = [dat[i][id_tag] for i in id_val]
