...
def sg_to_crystal_system(sg):
    if 1 <= sg <= 2:
        return 0, "triclinic"
    elif 3 <= sg <= 15:
        return 1, "monoclinic"
    elif 16 <= sg <= 74:
        return 2, "orthorhombic"
    elif 75 <= sg <= 142:
        return 3, "tetragonal"
    elif 143 <= sg <= 167:
        return 4, "trigonal"
...
class PygGraph(object):
    ...
    def atom_dgl_multigraph(
        ...
        # spg = atoms.get_spacegroup_info()
        # print(type(spg))
        # sg_number = spg["number"]
        # sg_symbol = spg["symbol"]
        ...
        sg_number = spg.space_group_number
        sg_symbol = spg.space_group_symbol
        ...
        crysys_id, crysys_name = sg_to_crystal_system(sg_number)
        
        g = Data(x=node_features, edge_index=edge_index, edge_attr=r,)
        g.label_attr = crysys_id
